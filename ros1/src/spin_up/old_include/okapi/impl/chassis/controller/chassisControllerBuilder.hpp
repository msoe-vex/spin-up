...
class ChassisControllerBuilder {
 public:
   ...
   */
  ChassisControllerBuilder& withMotors(
      const Motor& itopLeft, const Motor& itopRight, const Motor& ibottomRight,
   ...
   */
  ChassisControllerBuilder& withMotors(
      const MotorGroup& itopLeft, const MotorGroup& itopRight,
      const MotorGroup& ibottomRight, const MotorGroup& ibottomLeft);
   ...
   */
  ChassisControllerBuilder& withMotors(
      const Motor& ileft, const Motor& iright, const Motor& imiddle);
   ...
   */
  ChassisControllerBuilder& withMotors(
      const MotorGroup& ileft, const MotorGroup& iright, const Motor& imiddle);
  ...
  ChassisControllerBuilder& withSensors(
      const ADIEncoder& ileft, const ADIEncoder& iright);
   ...
   */
  ChassisControllerBuilder& withSensors(
      const IntegratedEncoder& ileft, const IntegratedEncoder& iright);
   ...
   * @return An ongoing builder.
   */
  ChassisControllerBuilder& withOdometry(
   ...
   */
  ChassisControllerBuilder& withOdometry(
      const ChassisScales& iodomScales,
      const StateMode& imode = StateMode::FRAME_TRANSFORMATION,
      const QLength& imoveThreshold = 0_mm,
      const QAngle& iturnThreshold = 0_deg);
   ...
   * @return An ongoing builder.
   */
  ChassisControllerBuilder& withChassisControllerTimeUtilFactory(
   ...
   * @return An ongoing builder.
   */
  ChassisControllerBuilder& withClosedLoopControllerTimeUtil(
      double iatTargetError = 50, double iatTargetDerivative = 5,
   ...
   * @return An ongoing builder.
   */
  ChassisControllerBuilder& withOdometryTimeUtilFactory(
      const TimeUtilFactory& itimeUtilFactory);
 ...
 private:
  ...
  std::shared_ptr<DefaultOdomChassisController> buildDOCC(
