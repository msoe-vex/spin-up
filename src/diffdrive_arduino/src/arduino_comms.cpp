...
{  
    for (auto topic_name : l_topic_names_) {
        l_publishers_.push_back(this->create_publisher<std_msgs::msg::Float32>(prefix_ + topic_name + postfix_, 10));
    }
                                                                                                                      
    for (auto topic_name : r_topic_names_) {
        r_publishers_.push_back(this->create_publisher<std_msgs::msg::Float32>(prefix_ + topic_name + postfix_, 10));
    }
    l_subscriber_ = this->create_subscription<v5_hal::msg::V5Motor>(prefix_ + l_topic_names_.front(), 10, std::bind(&ArduinoComms::l_topic_callback, this, std::placeholders::_1));
    r_subscriber_ = this->create_subscription<v5_hal::msg::V5Motor>(prefix_ + r_topic_names_.front(), 10, std::bind(&ArduinoComms::r_topic_callback, this, std::placeholders::_1));
}
